...
#if defined(FL_ALPHA)
UNITY_INSTANCING_BUFFER_START(Props_transparency)
	UNITY_DEFINE_INSTANCED_PROP(fixed, _Alpha)
UNITY_INSTANCING_BUFFER_END(Props_transparency)
#endif
...
inline half4 FLfragment(fl_vertex2fragment i) {
	...
	#if defined(FL_FOG)
		flatLight = CustomFog(vertex, flatLight);
		fogAlpha = flatLight.a;
	#endif
	...
	fixed4 result = flatLight + (light + ambientColor + directLight);
		...
		result.rgb = result.rgb * attenuation;
	#endif

	#if defined(FL_ALPHA)
		fixed alpha = UNITY_ACCESS_INSTANCED_PROP(Props_transparency, _Alpha);
		result.a = axisAlpha * gradientAlpha * fogAlpha * alpha;
	#endif
	return result;
}
