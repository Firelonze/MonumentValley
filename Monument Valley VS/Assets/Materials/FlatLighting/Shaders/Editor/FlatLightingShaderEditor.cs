...
public class FlatLightingShaderEditor : ShaderGUI {
    ...
    private MaterialProperty fogColor = null;
    private MaterialProperty fogBlending = null;
    private MaterialProperty fogOffset = null;

    protected MaterialEditor materialEditor;
	protected Material material;

    protected bool disableAxisSpaceSelection = false;
	protected Space selectedAxisSpace = Space.Local;
    ...
    private ValueWrapper<bool> useGPUInstancing = false;
    private ValueWrapper<bool> useFog = false;
	...
	protected virtual void FindProperties (MaterialProperty[] props) {
        ...
        fogBlending = FindProperty("_FogBlendingWidth", props);
        fogOffset = FindProperty("_FogCameraOffset", props);
	}
	...
	private void InitShaderPropertiesGUI() {
		...
		shouldShowGradientProperties = (isKeywordEnabled(Constants.Shader.GRADIENT_LOCAL_KEYWORD) || isKeywordEnabled(Constants.Shader.GRADIENT_WORLD_KEYWORD)) || shouldShowGradientProperties || useFog;
	...
	protected void ShowLightingProperties() {
		...
		if (shouldShowLightingProperties) {
			using (new UITools.GUIVertical(UITools.VGroupStyle)) {
				...
				GUILayout.Space(defaultSpace);
				using (new UITools.GUIDisable(IsShaderBaked(material) || disableAxisSpaceSelection)) {
    ...
    private void ShowGPUInstancingOption() {
        using (new UITools.GUIDisable(IsShaderBaked(material))) {
            UITools.ToggleHeader(useGPUInstancing, Labels.UseGPUInstancing);
        }
        if (useGPUInstancing) {
    ...
    protected void ShowGradientProperties() {
        using (new UITools.GUIVertical(UITools.VGroupStyle))
        {
            using (new UITools.GUIDisable(IsShaderBaked(material)))
            {
    ...
    protected void ShowFogProperties() {
        using (new UITools.GUIVertical(UITools.VGroupStyle))
        {
            using (new UITools.GUIDisable(IsShaderBaked(material)))
            {
                UITools.ToggleHeader(useFog, Labels.UseCameraFog);
            }
            ...
            if (useFog)
            {
                ...
                material.EnableKeyword(Constants.Shader.USE_FOG_KEYWORD);

                materialEditor.ShaderProperty(fogColor, Labels.FogColor);
                materialEditor.ShaderProperty(fogBlending, Labels.FogBlendingWidth);
                materialEditor.ShaderProperty(fogOffset, Labels.FogCameraOffset);
            }
            else
            {
                material.DisableKeyword(Constants.Shader.USE_FOG_KEYWORD);
            }
        }
    }
